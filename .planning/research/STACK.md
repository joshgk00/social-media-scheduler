...
**Domain:** Self-hosted social media scheduler with queue automation
...
```bash
# Core backend
npm install express@5 ioredis bullmq drizzle-orm postgres zod luxon pino pino-http
npm install argon2 express-session connect-redis helmet csrf-csrf otpauth uuid
npm install nodemailer sharp multer twitter-api-v2 twitter-text facebook-nodejs-business-sdk
npm install csv-parse csv-stringify @bull-board/express @bull-board/api
...
npm install -D @types/luxon @types/multer @types/nodemailer @types/uuid
npm install -D @types/facebook-nodejs-business-sdk
npm install -D drizzle-kit vite @vitejs/plugin-react
...
**For the scheduling worker (BullMQ):**
...
**For media storage (local filesystem):**
...
RUN apk add --no-cache python3 make g++  # For native addons (argon2, sharp)
WORKDIR /app
COPY package*.json ./
RUN npm ci
COPY . .
...
# Production stage
FROM node:22-alpine
RUN apk add --no-cache ffmpeg  # For video transcoding
WORKDIR /app
...
---
*Stack research for: self-hosted social media scheduler*
*Researched: 2026-04-07*
